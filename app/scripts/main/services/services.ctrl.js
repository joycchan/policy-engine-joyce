...
angular.module('policyEngine').controller('ServicesCtrl',
  function ($scope, $state, assignments) {
    ...
    $scope.$watch('services', function() {
      $scope.providerGroups = _.uniq(_.map($scope.services, function(service) { return service.group }), 'name');
      $scope.ruleSets = _.uniq(_.map($scope.services, function(service) { return service.ruleSet }), 'name');
